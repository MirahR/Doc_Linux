...
string selectTypeAttribut()
{
    ...
    {
        int choix;
        cout << " Merci de choisir un type d'attribut\n";
        cout << " 1. string\n";
        cout << " 2. int\n";
        cout << " 3. float\n";
        cout << " 4. double\n";
                                                 ...
                                                 vector<string> &type_attributs)
{
    ...
    for (int i = 0; i < attributs.size(); i++)
    {
        contenuFichier += type_attributs[i] + attributs[i] + ",";
    }
    contenuFichier.erase(contenuFichier.end() - 1, contenuFichier.end());

    contenuFichier += ")\n{\n";
    for (int i = 0; i < attributs.size(); i++)
        contenuFichier += "\tthis->" + attributs[i] + " = " + attributs[i] + ";\n";
}
contenuFichier += "}\n";
return contenuFichier;
}
                         ...
                         vector<string> &type_attributs)
{
    ...
    {
        string s;
        transform(attributs[i].begin(), attributs[i].begin() + 1, back_inserter(s), ::toupper);
        s += attributs[i].substr(1, attributs[i].size() - 1);
        contenuFichier += "\n" + type_attributs[i] +
                          nomClasse + "::" +
                          "get" + s + "() const \n{\n\treturn " + attributs[i] + ";\n}\n";
    }
                         ...
                         vector<string> &type_attributs)
{
    for (int i = 0; i < attributs.size(); i++)
    {
        ...
        contenuFichier += "\nvoid " +
                          nomClasse + "::" +
                          "set" + s + "(" + type_attributs[i] + " " + attributs[i] + ")\n
        {
            \n "
               "\tthis->" +
                attributs[i] + " = " + attributs[i] + ";\n}\n";
        }
        // for (auto e : attributs)
        // {
        //     string s;
    ...
    void generationFichierCpp(string nomClasse,
                              vector<string> & attributs,
                              vector<string> & type_attributs)
    {
        string nomFichier;
        ...
        generationInclusionConstructeurDefaut(nomClasse, contenuFichier, attributs, type_attributs);
        generationInclusionConstructeurParametres(nomClasse, contenuFichier, attributs, type_attributs);
        generationGetters(nomClasse, contenuFichier, attributs, type_attributs);
        generationSetters(nomClasse, contenuFichier, attributs, type_attributs);
    ...
    string generationFonctionMain(string nomClasse,
                                  string & contenuFichier,
                                  vector<string> & attributs,
                                  vector<string> & type_attributs)
    {
    ...
    {
        ofstream fichier_h(nomClasse + ".h");
